...
\abstract{The social sciences, particularly economics, political science, and psychology, have all taken their turn in the spotlight recently for failures of reproducibility of prominently published research. This article reviews the problems and discusses recent developments and proposed solutions. The problems of publication bias, specification searches (also called data mining or p-hacking), and failures to replicate published research have been known decades, but a recent surge of research has documented that the problems are still quite prevalent. This has led to a spate of a proposed solutions, including study registration, pre-registration of statistical analysis plans, results-blind reviewing, disclosure checklists, and journal requirements of data and code sharing. New methodological tools to test for publication bias, as well as more established tools such as meta-analysis and multiple hypothesis testing adjustment, have become more widely adopted. Lastly, technological improvements such as version control and dynamic documents have made a reproducible research workflow much simpler.}
...
the public at large \citep{merton1973sociology}. An important requirement for such scrutiny is that
...
power, it is essential that data, code, and analyses be transparent \citep{marwick_how}.

This review article is intended to be a source for empirical social
...
appropriately, including a few practical suggestions on how researchers can work collaboratively to obtain adequate sample size.
...
Although the blog tends to specialize in the life sciences, and significantly less money is involved in the social sciences than in medical and pharmaceutical research, there is no reason to believe that social science researchers are inherently
more benevolent. 
...
Many issues involving study design are related to transparent and reproducible research. For randomized trials, I refer readers to two excellent resources, \cite{randomizationtoolkit} and \cite{glennerster_running_2013}, which cover many aspects of how to design and implement a field trial. For studies that are not necessarily randomized trials, I recommend \cite{gertler_impact_2011}, or \cite{angrist2008mostly} for those with more statistics training. Here I briefly discuss one aspect of study design especially relevant for reproducible research: determining sample size through power calculations.
...
This means that when a low powered study does find an effect, the lower the likelihood that this effect is true in 
...
Alternatives to the standard power analysis have been suggested to combat these problems. For example, \cite{perugini2014safeguard} 
...
In many cases, these calculations will reveal the need for a larger sample size than some disciplines are used to handling. There are several ways to mitigate the impact of this. 
...
results. Thankfully, tools are available for researchers to combat
these problems.
...
Some specific workflow organization suggestions that should be valid regardless of code or operating system are listed below. Stata-specific suggestions are included in the appendix. 
\begin{itemize}
\item
...
\item
  Indent your code.
\item
    Once you post or distribute code or data, any and all changes require a new file name.
\item
  ...
  files) in an analysis file---it is better to only create the variables once so you know they're identical when used in different analysis files.
\item
...
In research, the devil truly is in the details. Whether assessing the validity of a research design or attempting to replicate a study, details of what exactly was done must be recorded and made available to other researchers. The exact details that are relevant will likely differ from field to field, but an increasing number of fields have
produced centralized checklists that describe (in excruciating detail) what disclosure is required of published studies. These checklists are not often published with the paper, but can be submitted with the original article so that reviewers can check that it has been completed. With nearly infinite and easy web storage, researchers can easily post these materials on their website even if journal editors insist on cutting their methods sections for space reasons. 
...
For researchers unfamiliar with any of the terms involved, I include a glossary of terms.
\begin{itemize}
\item
